...
class ML1MUserResponse(BaseModel):
    ...
    def parse_model_args(parser):
        ...
        - attn_n_head
        - hidden_dims
        - dropout_ratepath_to_data = "/content/drive/MyDrive/RL/HAC/ml1m/"
        - batch_norm
        - from BaseModel:
            - model_path
            - loss
    ...
    def _define_params(self, args, reader):
        ...
        # self.uEmb = nn.Embedding(6500, 33)
        # self.iEmb = nn.Embedding(3800, 33)
    ...
    def get_loss(self, feed_dict: dict, out_dict: dict):
        ...
        # loss
        print("self.sigmoid(preds): ", self.sigmoid(preds))
        print("target: ", target)
        # exit()
        
        loss = torch.mean(self.bce_loss(self.sigmoid(preds), target))
        print("loss: ", loss, "l2: ", reg, "l2*coef",self.l2_coef * reg) 
        self.loss.append(loss.item())
        # print("auc: ", roc_auc_score(self.sigmoid(preds), target))
