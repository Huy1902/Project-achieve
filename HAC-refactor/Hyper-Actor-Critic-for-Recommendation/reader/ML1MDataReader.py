...
class ML1MDataReader(BaseReader):
...
#         self.portrait_vocab = get_vocab_of_seq_feature(self.data['train'], 'user_protrait')
    ...
    def __getitem__(self, idx):
        ...
        '''
        user_ID, slate_of_items, user_mid, user_mid_history, sequence_id = self.data[self.phase].iloc[idx]
        ...
        feedback = eval(user_mid)
        # feedback = padding_and_clip(feedback, 10)    # test
        # print("******", user_ID, sequence_id)
        # print(np.array(user_ID).astype(int))
        record = {
            'timestamp': int(1),
    ...
    def get_item_list_meta(self, iid_list, from_idx = False):
        ...
        '''
        # iid_list = list(map(lambda x: self.item_vocab[x], iid_list))
        # iid_list = list(map(lambda x: self.item_vocab.get(x, 0), iid_list))
        features = []
        for iid in iid_list:
            if iid == 0:
                features.append(self.item_meta[iid])
            else:
                features.append(self.item_meta[iid])
